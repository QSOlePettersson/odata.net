...
namespace Microsoft.OData.Client
{
    ...
    internal class BatchActionResult<T> : BaseSaveResult
    {
        ...
        private readonly DataServiceActionQuerySingle<T>[] Queries;
        ...
        internal BatchActionResult(DataServiceContext context, string method, DataServiceActionQuerySingle<T>[] queries, SaveChangesOptions options, AsyncCallback callback, object state)
            : base(context, method, new DataServiceRequest[0], options, callback, state)
        {
                        ...
                        case ODataBatchReaderState.Operation:
                            ...
                            if (!insideChangeset)
                            {
                                ...
                                try
                                {
                                    if (exception == null)
                                    {
                                        DataServiceActionQuerySingle<T> query = this.Queries[queryCount];
                                        ResponseInfo responseInfo = this.RequestInfo.GetDeserializationInfo(null /*mergeOption*/);
                                        QueryComponents qc = query.QueryComponents();
                                        ...
                                        var requestUri = qc.Uri;
                                        DataServiceRequest request = new DataServiceRequest<T>(requestUri, qc, null);
                                        qresponse = new QueryOperationResponse<T>(this.currentOperationResponse.Headers, request, materializer);

                                        qresponse.StatusCode = (int)currentOperationResponse.StatusCode;
                                    }
                                }
                                ...
                                if (qresponse == null)
                                {
                                    if (this.Queries != null)
                                    {
                                        ...
                                        {
                                            qresponse = new QueryOperationResponse<T>(this.currentOperationResponse.Headers, request, MaterializeAtom.EmptyResults);
                                        }
                                        else
                                        {
                                            qresponse = new QueryOperationResponse<T>(this.currentOperationResponse.Headers, request, MaterializeAtom.EmptyResults);
